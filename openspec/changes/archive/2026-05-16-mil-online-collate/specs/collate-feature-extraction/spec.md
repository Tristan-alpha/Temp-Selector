...
The collate function SHALL receive the SGLangRunner instance through `functools.partial` so it can be shared across train and val DataLoaders without duplicating the engine.

#### Scenario: Shared extractor across loaders

- **WHEN** `train_mil()` creates one SGLangRunner and two DataLoaders (train + val)
- **THEN** both loaders use `partial(collate_fn, extractor=runner, ...)` with the same runner instance
